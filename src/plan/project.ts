...
import { planLibraryBundler } from './bundlers.js';
 ...
 * (see `resolveProjectFacts` in the resolve stage). Unresolved fields fall
 * back to the catalog defaults.
 */
export function planProject(input: ProjectPlanInput | ProjectOptions): ProjectPlan {
  const planInput = isProjectPlanInput(input) ? input : resolveProjectPlanInput(input);
  return createProjectPlan(planInput);
}
