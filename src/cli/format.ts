...
export function formatConfigSummary(
  ...
  const baseTemplate = options.template
    ? getBaseTemplate(options.template)
    : "vanilla";
  ...
  lines.push(
    formatRow(
      "Package manager",
      getPackageManagerName(options.packageManager),
      pmInherited,
    ),
  );
  ...
  if (getPackageManagerName(options.packageManager) === "pnpm") {
    ...
    lines.push(
      formatRow("↳ Version managed", versionManaged, pnpmVersionInherited, ""),
    );
  }
  ...
  // Testing
  const testing =
    options.testing ?? (projectType === "library" ? "vitest" : "none");
  ...
  if (!inherited) {
    lines.push(
      formatRow(
        "Editor config",
        options.ide === "none" ? "generic" : "generic + vscode",
      ),
    );
 ...
 */
export function formatMonorepoConfigSummary(
  options: MonorepoConfigOptions,
): string {
  const lines: string[] = [];
  const VALUE_COL = 27;
