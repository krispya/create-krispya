...
async function detectStandaloneConfigStrategy(root: string): Promise<ConfigStrategy> {
 ...
 */
export async function generateExpectedFiles(
    config: WorkspaceConfig
): Promise<Record<UpdateCategory, Record<string, File>>> {
 ...
 */
export function needsMigration(current: WorkspaceConfig, target: MigrationTarget): boolean {
...
export async function getMigrationPlan(
    ...
    const targetVersions =
        toLinter === 'biome' || toFormatter === 'biome'
            ? await resolveMonorepoRootPackageVersions({
                  linter: toLinter,
                  formatter: toFormatter,
              })
            : {};
    const biomeSchemaUrl =
        toLinter === 'biome' || toFormatter === 'biome'
            ? `https://biomejs.dev/schemas/${getResolvedPackageVersion(
                  targetVersions,
                  '@biomejs/biome'
              )}/schema.json`
            : '';
    ...
    if (toFormatter !== current.formatter) {
        ...
        const formatterSameAsLinter = (current.formatter as string) === (current.linter as string);
        ...
        const newFormatterSameAsLinter = (toFormatter as string) === (toLinter as string);
 ...
 */
export async function applyMigration(plan: MigrationPlan, root: string): Promise<void> {
 ...
 */
async function updateRootPackageJson(root: string, plan: MigrationPlan): Promise<void> {
    const pkgPath = join(root, 'package.json');
    const content = await readFile(pkgPath, 'utf-8');
    const pkg = JSON.parse(content) as {
